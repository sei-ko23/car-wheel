...
class Circle {
  ...
  calcMarksAngles(count, degAngle, startAngle = 0) {
    ...
    for (let i = 0; i < count; i++) {
      const angle =
        ((i * radAngle) / (count - 1) - addAngle - deg2Rad(startAngle)) * -1;
      angles.push(angle);
    }
  ...
  calcTextMarksAngles(count, degAngle, innerOffset, startAngle = 0) {
    ...
    for (let i = -count + 1; i <= 0; i++) {
      const angle =
        (i * radAngle) / (count - 1) + addAngle + deg2Rad(startAngle);
      angles.push(angle);
    }
  ...
  drawTextMarks(angles, step, offset, fractions = false, reverse = false) {
    ...
    for (let angle in angles) {
      const x =
        Math.cos(angles[angle]) * (radius - radius / length) -
        this.textMarksOffsetX;
      const y =
        Math.sin(angles[angle]) * (radius - radius / length) +
        this.textMarksOffsetY;
  ...
  drawMultiplier(multiplier, offsetY) {
    ...
    ctx.save();
    ctx.translate(this.x, this.y);
    ctx.font = `${this.markFontStyle} ${this.markFontSize / 1.75}px ${
      this.markFontFamily
    }`;
    ctx.fillStyle = this.markFontColor;
...
class AdditionalCircle extends Circle {
  ...
  constructor(options, side, mainCircleRadius) {
    ...
    const x =
      cw -
      (cw - mainCircleRadius / 2 - this.radius - this.circleBorderWidth / 1.25);
